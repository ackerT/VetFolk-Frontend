...
const BuscarMascota = () => {
  ...
  const handleViewDetails = (petId) => {
    navigate(`/admin/updateregisterpet/${petId}`);
  };
  ...
  const columns = [
    { field: 'id_mascota', headerName: 'ID Mascota', width: 100 },
    { field: 'nombre_especie', headerName: 'Especie', width: 100 },
    { field: 'nombre_raza', headerName: 'Raza', width: 130 },
    { field: 'genero', headerName: 'Género', width: 130 },
    { field: 'nombre_mascota', headerName: 'Nombre', width: 100 },
    { field: 'edad', headerName: 'Edad', width: 80 },
    { field: 'peso', headerName: 'Peso', width: 80 },
    { field: 'agresiva', headerName: 'Agresiva', 
      width: 100, valueGetter: (params) => (params.row?.agresiva ? 'Sí' : 'No') },
    { field: 'esterilizada', headerName: 'Esterilizada', width: 120, 
      valueGetter: (params) => (params.row?.esterilizada ? 'Sí' : 'No') },
    {
      field: 'expediente',
  ...
  return (
    <ThemeProvider theme={theme}>
      <Box sx={{ display: 'flex', flexDirection: 'column', marginLeft: '250px' }}>
        <AdminSideBar />
        <Box sx={{ flexGrow: 1, padding: 2 }}>
          <Typography variant="h5">
            Buscar Mascotas</Typography>
          <div>
            <label htmlFor="ownerQuery" style={{ marginRight: '05px' }}>Buscar por ID o Nombre del Propietario:</label>
            <input
              type="text"
              id="ownerQuery"
              value={ownerQuery}
          ...
          {selectedOwnerPets.length > 0 && (
            <div style={{ height: 360, width: '100%', marginTop: 20 }}>
