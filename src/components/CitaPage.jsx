...
export default function EditarCitaPage() {
  ...
  const fetchCitaById = async (idCita) => {
    try {
      const response = await axios.get(`http://localhost:3008/citas/obtener/${idCita}`);
      setFormData({
  ...
  const fetchEstados = async () => {
    try {
      const response = await axios.get('http://localhost:3008/estados/obtener');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:3008/citas/actualizar/${formData.idCita}`, {
