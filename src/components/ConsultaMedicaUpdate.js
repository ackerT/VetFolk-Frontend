...
export default function EditarConsultaMedica() {
  ...
  const fetchConsultaById = async (idConsulta) => {
    try {
      const response = await axios.get(`http://18.221.225.5/consultas/getby/${idConsulta}`);
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://18.221.225.5/consultas/actualizar/${formData.idConsulta}`,
        formDataToSubmit,
        { headers: { 'Content-Type': 'multipart/form-data' } }
      );

      if (response.status === 200) {
        const expedienteResponse = await axios.get(`http://18.221.225.5/expedientes/obtener/${formData.idExpediente}`);
        const idMascota = expedienteResponse.data[0]?.idMascota;
        if (idMascota) {
          setIsSubmitting(false);
          navigate(`/admin/buscar-expediente/${idMascota}`);
        } else {
          console.error('No se encontró el idMascota');
          setIsSubmitting(false);
        }
      } else {
    ...
    <>
      ...
      >
        ...
        <Box>
            ...
            {formData.imagenes.length > 0 ? (
              formData.imagenes.map((image, index) => (
                <Box key={index} sx={{ position: 'relative', width: '120px' }}>
                  {/* Verifica si es un archivo (File) o una URL */}
                  <img
                    src={image instanceof File ? URL.createObjectURL(image) : image} // Si es un archivo, usa createObjectURL
                    alt={`Imagen ${index + 1}`}
                  ...
                  <Button
                    variant="contained"
                    color="error"
                    onClick={() => handleRemoveImage(image)} // Elimina la imagen
                    sx={{
                      position: 'absolute',
                      top: '4px',
                      right: '4px',
          ...
          sx={{ 
            backgroundColor: '#2c6b6b', color: '#fff'}}>
            Agregar Imagen
          </Button>
        </Box>
      ...
      {/* Diálogo para agregar una nueva imagen */}
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        ...
        <DialogActions>
          <Button onClick={handleCloseDialog} sx={{ backgroundColor: '#2c6b6b', color: '#fff'}}>Cancelar</Button>
          <Button onClick={handleAddImage}  disabled={!newImage} sx={{ backgroundColor: '#2c6b6b', color: '#fff'}}>Agregar</Button>
        </DialogActions>
      </Dialog>
    </>
  );
