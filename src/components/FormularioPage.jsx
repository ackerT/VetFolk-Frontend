...
export default function Component() {
  ...
  useEffect(() => {
    const fetchCitas = async () => {
      try {
        const response = await fetch("http://localhost:3008/citas/obtener");
  ...
  const onSubmit = async (formData) => {
    ...
    const response = await fetch(`http://localhost:3008/citas/verificar?fecha=${nuevaFechaFormateada}&hora=${hora}&idServicio=${idServicio}`);
    ...
    try {
      const createResponse = await fetch("http://localhost:3008/citas", {
        method: "POST",
        headers: {
          ...
          >
            ...
            <Box component="form" onSubmit={handleSubmit(onSubmit)} noValidate>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  <DateCalendar value={fecha} onChange={cambiarFecha} disablePast  shouldDisableDate={(date) => date.day() === 0} />
                </Grid>
                <Grid item xs={12}>
                  <FormControl fullWidth>
