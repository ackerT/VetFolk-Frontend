...
export const GestionarRolesPage = () => {
    ...
    const obtenerPersonas = async () => {
        try {
            const response = await fetch('http://localhost:3008/personas/conroles'); // Reemplaza con la URL correcta de tu servicio
            if (response.ok) {
    ...
    const obtenerRoles = async () => {
        try {
            const response = await fetch('http://localhost:3008/roles'); // Reemplaza con la URL de tu servicio para obtener los roles
            if (response.ok) {
                const data = await response.json();
                setRoles(data);
            } else {
    ...
    const manejarCambiarRol = async () => {
        if (nuevoRol) {
            try {
                const response = await fetch(`http://localhost:3008/personas/${usuarioSeleccionado.idPersona}/actrol`, {
                    method: 'PUT',
                    headers: {
