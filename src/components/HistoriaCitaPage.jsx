...
export const HistoriaCitaPage = () => {
    ...
    const styles = {
        ...
        disabledButton: { backgroundColor: '#d3d3d3', color: 'white', textTransform: 'capitalize', cursor: 'not-allowed' }
    };
        ...
        const fetchData = async () => {
            try {
                const [responseCitas, responsePersonas, responseEstados, responseServicios] = await Promise.all([
                    fetch('http://localhost:3008/citas/obtener'),
                    fetch('http://localhost:3008/personas/obtenerus'),
                    fetch('http://localhost:3008/estados/obtener'),
                    fetch('http://localhost:3008/servicios')
                ]);
                ...
                const sortedCitas = dataCitas.sort((a, b) => new Date(b.fecha) - new Date(a.fecha));
                ...
                <Table stickyHeader>
                    ...
                    <TableBody>
                        {filteredCitas.map(cita => {
                            const esCitaPasada = new Date(cita.fecha) < new Date().setHours(0, 0, 0, 0);
                            return (
                                <TableRow key={cita.idCita}>
                                    ...
                                    <TableCell>
                                        {cita.comentarios && cita.comentarios.trim() !== '' ? cita.comentarios : 'Sin comentarios'}
                                    </TableCell>
                                    <TableCell>
                                        <Button
                                            style={esCitaPasada ? styles.disabledButton : styles.button}
                                            onClick={() => !esCitaPasada && handleEditClick(cita.idCita)}
                                        >
                                            Editar
                                        </Button>
                                    </TableCell>
